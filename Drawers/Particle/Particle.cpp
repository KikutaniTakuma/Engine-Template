...
Particle::Particle(uint32_t indexNum) :
	...
	isAnimation_(0.0f)
{
	for (int32_t i = 0; i < wvpMat.Size();i++) {
...
void Particle::CreateGraphicsPipeline() {
	...
	rootPrams[1].DescriptorTable.NumDescriptorRanges = 1;
	rootPrams[1].DescriptorTable.pDescriptorRanges = &range;
...
bool Particle::Colision(const Vector2& pos2D) const {
	Vector2 max;
	Vector2 min;
...
bool Particle::Colision(const Particle& tex2D) const {
	Vector3 max;
	Vector3 min;
	max.x = std::max_element(worldPos.begin(), worldPos.end(),
		[](const Vector3& left, const Vector3& right) {
